...
from robokudo_foundation_pose.Utils import make_mesh_tensors, nvdiffrast_render, transform_pts, \
    compute_crop_window_tf_batch, cv_draw_text, depth_to_vis
from robokudo_foundation_pose.learning.datasets.h5_dataset import ScoreMultiPairH5Dataset, TripletH5Dataset
from robokudo_foundation_pose.learning.datasets.pose_dataset import BatchPoseData
from robokudo_foundation_pose.learning.models.score_network import ScoreNetMultiPair
...
def vis_batch_data_scores(pose_data, ids, scores, pad_margin=5):
    ...
    for id in ids:
        ...
        zmin = pose_data.depthAs[id].detach().cpu().numpy().reshape(H, W).min()
        zmax = pose_data.depthAs[id].detach().cpu().numpy().reshape(H, W).max()
        depthA_vis = depth_to_vis(pose_data.depthAs[id].detach().cpu().numpy().reshape(H, W), zmin=zmin, zmax=zmax, inverse=False)
        depthB_vis = depth_to_vis(pose_data.depthBs[id].detach().cpu().numpy().reshape(H, W), zmin=zmin, zmax=zmax, inverse=False)
...
@torch.no_grad()
def make_crop_data_batch(render_size, ob_in_cams, mesh, rgb, depth, K, crop_ratio, normal_map=None,
                         mesh_diameter=None, glctx=None, mesh_tensors=None, dataset: TripletH5Dataset=None, cfg=None):
...
class ScorePredictor:
    ...
    def predict(self, rgb, depth, K, ob_in_cams, normal_map=None, get_vis=False, mesh=None, mesh_tensors=None, glctx=None, mesh_diameter=None):
        ...
        def find_best_among_pairs(pose_data:BatchPoseData):
            rk_logger.info(f"pose_data.rgbAs.shape[0]: {pose_data.rgbAs.shape[0]}")
            ids = []
            scores = []
