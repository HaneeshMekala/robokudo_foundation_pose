...
from robokudo_foundation_pose.learning.datasets.pose_dataset import BatchPoseData
from robokudo_foundation_pose.learning.models.refine_network import RefineNet
...
def make_crop_data_batch(render_size, ob_in_cams, mesh, rgb, depth, K,
                         crop_ratio, xyz_map, normal_map=None,
                         mesh_diameter=None, cfg=None, glctx=None,
                         mesh_tensors=None, dataset: PoseRefinePairH5Dataset = None):
    rk_logger.info("Welcome make_crop_data_batch")
    H, W = depth.shape[:2]

    # compute crop transforms in pixels for output size (H, W)
    render_h, render_w = render_size[1], render_size[0]    # cfg says (W,H); kornia expects (H,W)
    tf_to_crops = compute_crop_window_tf_batch(
        pts=mesh.vertices, H=H, W=W, poses=ob_in_cams, K=K,
        crop_ratio=crop_ratio, out_size=(render_h, render_w),
        method="box_3d", mesh_diameter=mesh_diameter)

    rk_logger.info("make tf_to_crops done")
    ...
    # bbox covering the whole crop in crop-space
    ...
    rgb_rs = torch.cat(rgb_rs, dim=0).permute(0, 3, 1, 2) * 255     # (B,3,H,W)
    xyz_map_rs = torch.cat(xyz_map_rs, dim=0).permute(0, 3, 1, 2)   # (B,3,H,W)

    # expand K to (B,3,3) for later indexing
    Ks = torch.as_tensor(K, device="cuda", dtype=torch.float).reshape(1, 3, 3).expand(B, 3, 3)
    if cfg["use_normal"]:
        normal_rs = torch.cat(normal_rs, dim=0).permute(0, 3, 1, 2)

    rk_logger.info("render done")

    # kornia expects dsize=(H,W)
    dsize_hw = (render_h, render_w)
    ...
    rgbBs = kornia.geometry.transform.warp_perspective(
            ...
            tf_to_crops, dsize=dsize_hw, mode="bilinear", align_corners=False)

    rgbAs = rgb_rs if rgb_rs.shape[-2:] == cfg["input_resize"] else \
            kornia.geometry.transform.warp_perspective(rgb_rs, tf_to_crops, dsize=dsize_hw, mode="bilinear", align_corners=False)

    xyz_mapAs = xyz_map_rs if xyz_map_rs.shape[-2:] == cfg["input_resize"] else \
            kornia.geometry.transform.warp_perspective(xyz_map_rs, tf_to_crops, dsize=dsize_hw, mode="nearest", align_corners=False)
    ...
    xyz_mapBs = kornia.geometry.transform.warp_perspective(
            ...
            tf_to_crops, dsize=dsize_hw, mode="nearest", align_corners=False)   # (B,3,H,W)

    if cfg["use_normal"]:
        normalAs = kornia.geometry.transform.warp_perspective(normal_rs, tf_to_crops, dsize=dsize_hw, mode="nearest", align_corners=False)
        normalBs = kornia.geometry.transform.warp_perspective(
                ...
                tf_to_crops, dsize=dsize_hw, mode="nearest", align_corners=False)
    else:
        normalAs = None
        normalBs = None

    rk_logger.info("warp done")

    mesh_diameters = torch.ones((B), dtype=torch.float, device="cuda") * mesh_diameter
    ...
    rk_logger.info("pose batch data done")
    return pose_data


class PoseRefinePredictor:
    def __init__(self,):
        rk_logger.info("welcome")
        self.amp = True
        ...
        rk_logger.info(f"Using pretrained model from {ckpt_dir}")
    ...
    def predict(self, rgb, depth, K, ob_in_cams, xyz_map, normal_map=None, get_vis=False, mesh=None, mesh_tensors=None,
        ...
        rk_logger.info(f"ob_in_cams:{ob_in_cams.shape}")
        tf_to_center = np.eye(4)
        ob_centered_in_cams = ob_in_cams
        mesh_centered = mesh

        rk_logger.info(f"self.cfg.use_normal:{self.cfg.use_normal}")
        if not self.cfg.use_normal:
            normal_map = None

        crop_ratio = self.cfg["crop_ratio"]
        rk_logger.info(f"trans_normalizer:{self.cfg["trans_normalizer"]}, rot_normalizer:{self.cfg["rot_normalizer"]}")
        bs = 1024
        ...
        for _ in range(iteration):
            rk_logger.info("making cropped data")
        ...
        if get_vis:
            ...
            for id in range(0, len(B_in_cams)):
                rgbA_vis = (pose_data.rgbAs[id]*255).permute(1,2,0).detach().cpu().numpy()
                rgbB_vis = (pose_data.rgbBs[id]*255).permute(1,2,0).detach().cpu().numpy()
                row = [rgbA_vis, rgbB_vis]
                H,W = rgbA_vis.shape[:2]
                if pose_data.depthAs is not None:
                    depthA = pose_data.depthAs[id].detach().cpu().numpy().reshape(H,W)
                    depthB = pose_data.depthBs[id].detach().cpu().numpy().reshape(H,W)
                elif pose_data.xyz_mapAs is not None:
                    depthA = pose_data.xyz_mapAs[id][2].detach().cpu().numpy().reshape(H,W)
                    depthB = pose_data.xyz_mapBs[id][2].detach().cpu().numpy().reshape(H,W)
