...
class RefineNet(nn.Module):
  def __init__(self, cfg=None, c_in=4, n_view=1):
    super().__init__()
    self.cfg = cfg or {}
    use_bn = self.cfg.get('use_BN', True)
    ...
    self.encodeA = nn.Sequential(
      ...
      ConvBNReLU(64, 128, kernel_size=3, stride=2, norm_layer=norm_layer),
    ...
    rot_rep = self.cfg.get('rot_rep', 'axis_angle')
    rot_out_dim = 3 if rot_rep == 'axis_angle' else 6 if rot_rep == '6d' else 3
