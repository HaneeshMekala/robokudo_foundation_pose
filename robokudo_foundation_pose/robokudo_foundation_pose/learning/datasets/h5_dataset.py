...
from robokudo_foundation_pose.Utils import depth2xyzmap_batch
...
class PairH5Dataset(torch.utils.data.Dataset):
    def __init__(self, cfg, h5_file, mode="train", max_num_key=None, cache_data=None):
        ...
        rk_logger.info(f"self.h5_file:{self.h5_file}")
        self.n_perturb = None
        self.H_ori = None
        self.W_ori = None
        ...
        if self.mode=="test":
            ...
            if os.path.exists(key_file):
                ...
                rk_logger.info(f"object_keys loaded#:{len(self.object_keys)} from {key_file}")
                ...
                with h5py.File(h5_file, "r", libver="latest") as hf:
                    for k in hf:
                        ...
                        if max_num_key is not None and len(self.object_keys)>=max_num_key:
                            rk_logger.info("break due to max_num_key")
                            break

            rk_logger.info(f"self.object_keys#:{len(self.object_keys)}, max_num_key:{max_num_key}")
                ...
                for k_perturb in group:
                    ...
                    if self.H_ori is None:
                        if "H_ori" in group[k_perturb]:
                            ...
                            self.W_ori = 720
                self.n_perturb = cnt
                rk_logger.info(f"self.n_perturb:{self.n_perturb}")
        ...
        if mode!="test":
            with h5py.File(h5_file, "r", libver="latest") as hf:
                ...
                for key_perturb in group:
                    ...
                    rk_logger.info(f"n_view:{self.cfg["n_view"]}")
                    ...
                    rk_logger.info(f"self.trans_normalizer:{self.trans_normalizer}, self.rot_normalizer:{self.rot_normalizer}")
                    break
